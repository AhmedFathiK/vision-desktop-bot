...
def handle_dialogs(bot):
    ...
    # 3. Handle "Problem with Shortcut" dialog (known)
    if handle_problem_with_shortcut_dialog(bot):
        return

    # 4. Handle unknown/generic dialogs
...
def handle_cant_find_file_dialog(bot):
    ...
    if bot.find("cant_find_file_dialog_dark", matching=0.97, waiting_time=500):
        logging.info("Detected 'Cannot find file' dialog (Dark). Dismissing...")
        bot.key_enter()
        return True
    elif bot.find("cant_find_file_dialog_light", matching=0.97, waiting_time=500):
        logging.info("Detected 'Cannot find file' dialog (Light). Dismissing...")
        bot.key_enter()
        return True
    return False
...
def handle_confirm_save_as_dialog(bot):
    ...
    if windows:
        logging.info("Detected 'Confirm Save As' dialog. Confirming...")
        windows[0].activate()
        bot.type_keys(["alt", "y"])
        return True
    return False

def handle_problem_with_shortcut_dialog(bot):
    """Handles the 'Problem with Shortcut' dialog by clicking Yes."""
    windows = gw.getWindowsWithTitle("Problem with Shortcut")
    if windows:
        logging.info("Detected 'Problem with Shortcut' dialog. Clicking Yes...")
        try:
            windows[0].activate()
            sleep(0.5)
            ...
            logging.warning(f"Failed to handle 'Problem with Shortcut' dialog: {e}")
    return False
...
def handle_unknown_dialogs(bot):
    ...
    try:
        ...
        if not is_main_notepad:
            logging.warning(f"Unexpected active window detected: '{active_window.title}'")
            ...
            if notepad_windows:
                logging.info("Notepad window found in background. Attempting to switch back...")
                try:
                    notepad_windows[0].activate()
                    sleep(0.5)
                    if " - Notepad" in gw.getActiveWindow().title:
                        logging.info("Successfully switched back to Notepad.")
                        return
                except Exception as e:
                    logging.warning(f"Could not activate Notepad directly (might be blocked by dialog): {e}")

            # Special handling for "Notepad" dialogs (likely "Do you want to save?" or errors)
            if active_window.title == "Notepad":
                logging.info("Likely 'Do you want to save?' dialog. Attempting 'Don't Save' (Alt+N)...")
                bot.type_keys(["alt", "n"])
                sleep(1)
                ...
                if not active_window or " - Notepad" in active_window.title:
                    logging.info("Successfully dismissed 'Notepad' dialog.")
                    return

            # Try to interact with it to bypass/close
            # Strategy 1: Press Enter (Common for "OK", "Yes", "Save", "Confirm")
            logging.info("Attempting to bypass with Enter...")
            bot.key_enter()
            sleep(1)
            ...
            if not is_main_notepad:
                ...
                logging.info("Enter failed. Attempting to dismiss with Esc...")
                bot.key_esc()
                sleep(1)
            ...
            if active_window and (" - Notepad" in active_window.title):
                logging.info("Successfully returned focus to Notepad.")
            else:
                logging.error("Failed to dismiss unexpected dialog. Manual intervention may be required.")
                
    except Exception as e:
        logging.error(f"Error handling dialogs: {e}")
